...
import {
  ...
  AlignJustify, ChevronDown, ChevronLeft, ChevronRight, Check,} from "lucide-react";
import { useNavigate } from "react-router-dom";

const MARKET_STYLES = {
  KOSPI:  { bg: "bg-blue-50",    text: "text-blue-700",   border: "border-blue-200"   },
  KOSDAQ: { bg: "bg-violet-50",  text: "text-violet-700", border: "border-violet-200" },
  NYSE:   { bg: "bg-amber-50",   text: "text-amber-700",  border: "border-amber-200"  },
  NASDAQ: { bg: "bg-cyan-50",    text: "text-cyan-700",   border: "border-cyan-200"   },
};

function MarketBadge({ market }) {
  const s = MARKET_STYLES[market];
  if (!s) {
    return (
      <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold border bg-gray-100 text-gray-600 border-gray-200">
        {market}
      </span>
    );
  }
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-bold border ${s.bg} ${s.text} ${s.border}`}>
      {market}
    </span>
  );
}
...
export default function StockList({ stocks, watchlist, onToggle }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "Growth") list = list.filter(s => s.changePct > 0);
    if (filter === "Fall")   list = list.filter(s => s.changePct < 0);
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              {dropdownOpen && (
                <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-gray-100 rounded-xl shadow-lg z-30 py-1">
                  {FILTERS.map(({ label, icon }) => (
                    ...
                    >
                      {icon}
                      <span className={filter === label ? "font-semibold text-gray-900" : ""}>{label}</span>
                      {filter === label && <Check size={14} className="ml-auto text-green-500" />}
                    </button>
                  ))}
                </div>
              )}
              ...
              <tbody>
                {pageItems.length === 0 ? (
                  ...
                  pageItems.map((stock, i) => {
                    ...
                    const starred = watchlist[stock.id];
                      ...
                      >
                        <td className="pl-5 pr-2 py-5 ">
                          <button
                            ...
                            <Star
                              size={17}
                              fill={starred ? "#22c55e" : "none"}
                              stroke={starred ? "#22c55e" : "#d1d5db"}
                            />
                          </button>
                        </td>
                        ...
                        <td className="px-4 py-5"><MarketBadge market={stock.market} /></td>
        ...
        <div className="flex items-center justify-center gap-2 mt-8 flex-wrap">
          ...
          {visiblePages.map(n => (
            <button
              key={n}
              onClick={() => setPage(n)}
              className={`w-9 h-9 rounded-lg text-sm font-semibold transition ${safePage === n ? "bg-gray-900 text-white" : "text-gray-600 hover:bg-gray-100"}`}
            >
              {n}
            </button>
          ))}
